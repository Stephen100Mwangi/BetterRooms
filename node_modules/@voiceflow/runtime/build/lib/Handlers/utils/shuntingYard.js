...
exports.regexExpression = exports.evaluateExpression = void 0;
const lodash_1 = __importDefault(require("lodash"));
const workerpool_1 = __importDefault(require("workerpool"));
const evalExpression = async (expression, variables = {}) => {
    const exp = expression.split('\n');
    // start js script as dedicated worker
    const mathJsWorkerPool = workerpool_1.default.pool(`${__dirname}/mathJsWorker.js`, { maxWorkers: 1 });
    ...
    try {
        ...
        for (const i in postfixExpression) {
            if (typeof postfixExpression[i] === 'string' && postfixExpression[i].match(/^(\+|-|\*|\/|\^|==|&&|\|\||>|>=|<|<=|!|!=)$/)) {
                ...
                const operand1 = postfixExpression[i] === '!' ? null : tokenStack.pop();
                const result = evaluate(operand1, operand2, postfixExpression[i]);
...
const popOpStackUntil = (RPN, operatorStack, addToRPN, condition, isRightBracket = false) => {
...
const shuntingYard = async (expression, variables) => {
    const RPN = [];
    ...
    while (i < expression.length) {
        ...
        if (currToken.match(/[0-9]/) || (currToken === '-' && expression[i + 1] && expression[i + 1].match(/[0-9]/))) {
            ...
            popOpStackUntil(RPN, operatorStack, false, (topOp) => {
                return topOp === '(';
            }, true);
        }
        else if (currToken.match(/^(\+|-|\*|\/|\^|!)$/)) {
            ...
            const scope = lodash_1.default.cloneDeep(variables);
...
exports.evaluateExpression = async (expression, variables) => {
    try {
...
exports.regexExpression = (expression) => {
    if (lodash_1.default.isNumber(expression))
        return expression;
